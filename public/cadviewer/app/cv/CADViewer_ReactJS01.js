...
//      cvjs_loadCADViewerLanguage("English", ServerUrl+"/assets/cadviewer/app/cv/cv-pro/language_table/cadviewerProLanguage.xml");
    ...
    /** FIXED POP-UP MODAL  **/
      ...
      my_cvjsPopUpBody += "<div class=\'cvjs_modal_1\' onclick=\'cvjs_zoomHere();\'>Zoom<br>Here<br><i class=\'fa fa-search-plus\'></i></div>";
...
//  window.alert(graphicalObject);
	...
	function cvjs_graphicalObjectOnChange(type, graphicalObject, spaceID){
	  // do something with the graphics object created! 
		window.alert(" cvjs_graphicalObjectOnChange: "+type+" "+graphicalObject+" "+spaceID+" "+graphicalObject.toLowerCase().indexOf("space"));
		...
		if (type == 'Create' && graphicalObject.toLowerCase().indexOf("space")>-1 && graphicalObject.toLowerCase().indexOf("circle")==-1){
			...
			window.alert("This is the SVG path: "+myobject.path)
